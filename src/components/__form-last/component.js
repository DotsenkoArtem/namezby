...
function setupAlert(sendStatus, req, enableSound) {
  const alertMessage = {
    ...
    limitExceeded: {
      ...
      content: `К сожалению произошла ошибка. Пожалуйста, повторите снова.`,
      icon: 'error'
    },
    errorHttp: {
      title: "ошибка",
  ...
  let formAlertIcon = document.createElement("div");
  formAlertIcon.className = "form-alert__icon";
  ...
  formAlertContent.append(formAlertIcon, formAlertTitle);
  formAlert.append(formAlertContent, formAlertClose);
  wrapper.append(formAlert);

  // Setup alert status
  formAlertIcon.style.backgroundImage = `url(img/icons/icon-${alertMessage[sendStatus].icon}.svg)`
...
function removeAlert(submitBtn) {
  ...
  if (formAlert) {
    formAlert.style = ``;
    formAlert.style.animation = `slideOutDown ${duration}ms ease-in forwards`;
    setTimeout(() => {
      formAlert.remove();
